#!/usr/bin/env python3
"""
SMD Component Detector v5 — WHITE-STRIP DETECTION (RGB ONLY)
=============================================================
Complete rewrite:
  1. ALL frames are RGB — NO BGR conversion anywhere
  ...
  5. PIL used for image saving — guaranteed RGB output
  6. cv2.imshow converts RGB→BGR ONLY for display
...
def grab(cam):
    """Returns an RGB frame (always), or None on failure."""
    if USE_PICAMERA:
        arr = cam.capture_array()   # arrives as RGB888 — keep as RGB
        return arr
...
# ─────────────────────────────────────────────────────────────────────────────
#  DETECTION  — WHITE MASK on RED BACKGROUND (RGB input)
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    print("=" * 64)
    print(f"  Strip Detector v5 — RGB ONLY | {MAX_CAPTURES}-image session")
    ...
    try:
        while total_captures < MAX_CAPTURES:
            ...
            bbox, score, contour = find_component(frame_rgb)

            # Convert to BGR for display only
            ...
            if bbox is not None and score >= CONFIDENCE_THRESHOLD:
                ...
                if confirm_count >= CONFIRM_FRAMES:
                    ...
                    # Grab a fresh, settled frame (RGB)
                    capture_frame = grab(cam) or frame_rgb
                    ...
                    if cropped is not None and cropped.size > 0:
                        ...
                        if fc.size > 0:
                            save_rgb_image(fc, path)
                        else:
                            save_rgb_image(capture_frame, path)

                    total_captures += 1
                    print(f"[✓] Saved: {path}  ({total_captures}/{MAX_CAPTURES})")

                    # Banner (display in BGR)
                    cb_bgr = cv2.cvtColor(capture_frame, cv2.COLOR_RGB2BGR)
                    cv2.putText(cb_bgr,
                    ...
                    while time.time() < cl_end:
                        ...
                        if f is not None:
                            ...
                            draw_hud(f_bgr, "WAIT_GONE", 0, 0, total_captures)
                            cv2.imshow("Strip Detector", cv2.resize(f_bgr, (1280, 720)))
                        if cv2.waitKey(1) & 0xFF == ord('q'):
        ...
        while time.time() < deadline:
            ...
            if f is not None:
                f_bgr = cv2.cvtColor(f, cv2.COLOR_RGB2BGR)
                draw_hud(f_bgr, "DONE", 0, 0, total_captures)
                cv2.imshow("Strip Detector", cv2.resize(f_bgr, (1280, 720)))
                if SHOW_DEBUG_MASK and _last_mask is not None:
                    cv2.imshow("White Mask",
                               cv2.resize(_last_mask, (640, 480)))
